...
export default function PlayingList({ games }) {
  ...
  useEffect(() => {
    console.log("PlayingList received games:", games);
    setLocalGames(games || []);
  }, [games]);
  ...
  const handleRemoveGame = async (gameId, gameName) => {
    ...
    try {
      // Make DELETE request to remove the game from Playing list
      await axios.delete(`/users/playing/${gameId}`);
      
      // Update local state after successful deletion
      setLocalGames(prevGames => prevGames.filter(game => game.game_id !== gameId));
      
      // Show success message
      setSuccessMessage(`"${gameName}" successfully removed from playing list`);
      ...
      console.error('Error removing game from playing list:', err);
  ...
  return (
    <div className="games-list-container">
      ...
      {localGames.length === 0 ? (
        <p className="empty-list-message">Your playing list is empty.</p>
      ) : (
        <div>
          {localGames.map((game, i) => {
            // Log each game in the map to see its structure
            console.log(`Game ${i} in playing list:`, game);
